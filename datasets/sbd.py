...
class SBDDataset(BaseDataset):
    def __init__(self, root_dir, split='train', transform=None, target_transform=None):
        super(SBDDataset, self).__init__(root_dir, transform, target_transform)
        self.split = split
        self.root_dir = root_dir
        
        # SBD数据集使用与VOC2012相同的类别
        self.class_names = [
    ...
    def _load_dataset(self):
        """加载数据集"""
        # 读取数据集分割文件
        split_file = os.path.join(self.root_dir, 'dataset', f'{self.split}.txt')
        with open(split_file, 'r') as f:
            self.image_ids = [line.strip() for line in f.readlines()]
        
        # 构建图像和掩码路径
        self.images = [
            os.path.join(self.root_dir, 'img', f'{image_id}.jpg')
            for image_id in self.image_ids
        ]
        self.masks = [
            os.path.join(self.root_dir, 'cls', f'{image_id}.png')
            for image_id in self.image_ids
        ]
